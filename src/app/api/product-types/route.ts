...
export async function GET() {
    try {
        const data = await cache.productType.scan();
...
export async function POST(req: NextRequest) {
    try {
        ...
        const parsed = createProductTypeSchema.parse(body);

        const slug = slugify(parsed.name);

        const existingData = await queries.productType.get({ slug });
        if (existingData)
            throw new AppError(ERROR_MESSAGES.CONFLICT, "CONFLICT");

        const [data] = await Promise.all([
            queries.productType.create({ ...parsed, slug }),
            cache.subcategory.remove(parsed.subcategoryId),
