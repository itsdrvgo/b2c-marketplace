...
export async function GET(_: NextRequest, { params }: RouteProps) {
    try {
        ...
        const data = await cache.productType.get(id);
...
export async function PATCH(req: NextRequest, { params }: RouteProps) {
    try {
        ...
        const parsed = updateProductTypeSchema.parse(body);

        const existingData = await cache.productType.get(id);
        if (!existingData)
        ...
        if (slug !== existingData.slug) {
            const existingDataBySlug = await queries.productType.get({ slug });
            if (existingDataBySlug)
                throw new AppError(ERROR_MESSAGES.CONFLICT, "CONFLICT");
        }

        const [data] = await Promise.all([
            queries.productType.update(id, { ...parsed, slug }),
            cache.productType.remove(id),
        ]);
...
export async function DELETE(_: NextRequest, { params }: RouteProps) {
    try {
        ...
        const [data] = await Promise.all([
            queries.productType.delete(id),
            cache.productType.remove(id),
            cache.subcategory.remove(existingData.subcategoryId),
